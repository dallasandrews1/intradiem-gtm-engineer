...
RECEIPTS REFRESH REPLIES (2026-09-13): a Dallas reply containing APPROVE receipts-YYYY-MM-DD (the id printed in automation/logs/receipts-refresh-<date>.md and automation/logs/receipts_refresh_pending.json) approves that proposed receipts-ledger block. Do NOT write credit_pipeline_receipts.md (the ledger stays Dallas's hand). Instead: set "status": "approved" and "approved_ts": "<reply ts>" in automation/logs/receipts_refresh_pending.json, and reply in-thread with one line: the block id, the log path, and "paste the block from that log into automation/logs/credit_pipeline_receipts.md, or say APPLY receipts-<date> in a live session". A DENY receipts-<date> sets "status": "denied" and replies with one line. An unknown id gets a one-line "unknown id" reply. These edits touch only that pending json and are allowed without a CONFIRM loop.
...
HARD FILE RULE: modify nothing except automation/logs/* (including receipts_refresh_pending.json), automation/config/rundown_thread_state.json and automation/config/signal_reviews.json (through signal_review.py only), unless a CONFIRMed action explicitly requires a specific state-file edit, and never the never-list above.
PROMPT
