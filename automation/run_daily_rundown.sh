...
- Also read automation/logs/signal-review-<today>.md (signal_review.py, runs in the nightly sync; the live loop for war-room signals). If it lists signals under "Awaiting review", carry ONE line in the DM under what fired: "N war-room signals awaiting your review, reply APPROVE <id> or DENY <id> in this thread", then list each as one short line: id, company, family, date, the quote cut to 12 words, the URL. Never carry parked signals into the DM; note their count in the log file only. If the file is absent, say nothing about it.
