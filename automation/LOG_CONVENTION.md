...
- `<log-family>` matches the log filename family: `war-room`, `meeting-capture`, `competitor-displacement`, `gate-integrity`, `deliverability-watch`, `credit-check`, `swarm-health`, `agent-architect`, `stars-refresh-watch`, `action-brief`, `daily-rundown`.
