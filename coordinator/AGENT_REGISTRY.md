...
| `com.dallasandrews.gtm.accounthealth` | Mondays 7:10 | Churn-risk account health: refreshes evidence (Success Plan notes, adoption deck, Sales Nav alerts, Otter, PMO tracker export) and runs the deterministic scorer; reason codes with evidence, overdue items | `automation/logs/account-health-*.md` (rundown source 15) | **STAGED 2026-09-10, NOT LOADED** — plist in automation/, log-only, digest job separate and unbuilt |
| `com.dallasandrews.gtm.ownerdigest` | Mondays 7:30 | Save-plan owner digest: PMO tracker rows + account-health log grouped by owner; Outlook DRAFTS for routed owners (Inger, Amy), Dallas's block carried by the rundown | `automation/logs/owner-digest-*.md` (rundown source 16) | **STAGED 2026-09-10, NOT LOADED** — draft-only, never sends, unrouted owners get nothing |
...
| `com.dallasandrews.gtm.repcampaignhook` | Hourly (:25), after the relay | Pre-send hook for rep-built lemlist campaigns, the gap that let Jack's hand-loaded ST Water send 4 bounces on 2026-09-04. Step 1 (`rep_campaign_hook.py`, deterministic): discovers campaign ids missing from `campaign_channel_map`, ADDS them routed by campaign sender (its one write outside logs), checks every lead for a Clay Audiences record by email then LinkedIn URL, runs the address-pattern checks (reversed name order, collapsed hyphen, domain drift, one person with two addresses), and writes a DRY-RUN credit estimate against the 200 line with staged workflow inputs. Step 2 (Claude, only when step 1 finds something): `lemlist-lead-integrity` sweep scoped to the new campaigns plus the free Clay bridge on no-record leads. Never runs a workflow, edits a lead, or changes campaign state. On demand: `run_rep_campaign_hook.sh --campaign cam_x`; tests `python3 automation/test_rep_campaign_hook.py` (9/9) | `automation/logs/rep-campaign-hook-*.md`, `automation/staging/rep_hook/` | **LOADED 2026-09-04** — muted into the rundown; silence on empty, no dated log when no rep built a campaign |
| `com.dallasandrews.gtm.accountfacts` | Sunday 06:00 | Weekly account-facts refresh (live loop component 4, spec motions/live_loop/LIVE_LOOP_SPEC.md). Step 1 (deterministic): `account_facts_merge.py --select` picks universe domains with no filing-grade fact or one older than 90 days. Step 2 (Claude): fans out `automation/prompts/account_facts_researcher.txt` three accounts per signal-researcher agent on the strongest model, JSON only, into `automation/inbox/account_facts/<date>/`. Step 3 (deterministic): `account_facts_merge.py` validates every citation (URL + verbatim quote, aggregators rejected, dates parse) and merges into `tam-outbound-engine/data/account_facts.json`; DRY RUN until `automation/config/account_facts.json` merge=true. Never DMs, never spends credits, never edits the engine CSVs. | `automation/logs/account-facts-*.md` | **plist written 2026-09-11, NOT LOADED** until Dallas says so |
...
## partner-briefs-loop (RETIRED Sep 1 2026, same day it was built)
Watches #gtm-partner-briefs for account-name requests from the partner team (Frank Ciccone first), runs the briefing engine per request (gate from local SF/pre-pipeline/denylist files, signal-researcher fan-out, Clay free path 0 credits, build_briefs.py, partner-safe deploy only), replies in thread with links. DRY RUN until "live": true in automation/config/partner_briefs.json. Wrapper: automation/run_partner_briefs.sh. Log: automation/logs/partner-briefs-<date>.md (rundown source). Inherits governing rules 1-5: never DMs Dallas, logs with evt anchors, no sends, no credit spend beyond the free path, internal pages never deployed. RETIRED Sep 1 2026 before ever running live: the shared Claude project plus the two skills gave Frank branded briefs and one-pagers himself, the remaining need turned out to be routing context (who registered an account, what stage, which AE), and a scheduled Claude job was the wrong shape for it. Replaced by a static partner account lookup page. Two blockers also proved it could not run as built: unattended sessions cannot write to automation/logs or config without pre-approved permissions, and the wrapper's working directory is the main checkout, where motions/partner_channel/briefs does not exist (agents branch only).
