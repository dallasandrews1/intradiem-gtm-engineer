...
1. Pull the account's strike plan from the TAM engine. Run it yourself, first, before writing anything: `python3 tam-outbound-engine/account_engine.py --plan <domain>` (or `get_strike_plan(domain)` via the intradiem-tam MCP, or read `account_plays.json`, when the CLI is not reachable). Take the fit score, the dimension breakdown, the tier and the agent count from what it prints, verbatim. Never compose a score from the rubric, never adjust a dimension, never promote a tier. If the domain is not in `data/tam_accounts.csv` the engine cannot score it: say so and write the plan with no fit score rather than inventing one.
...
- **Tech-stack rule (2026-09-05).** A platform name (NICE, Five9, Genesys, Verint, Calabrio, Aspect, Amazon Connect, Talkdesk) may appear in copy as an observed fact ("on top of NICE", "we sit alongside Genesys") only when the account's CSV row carries a source URL and an observed date under 12 months for that platform (FULL mode step 1b). Anything else stays out: the copy says "WFM" or "your ACD" and never names a vendor. The engine enforces the same rule in its own copy through `tech_for_copy`.
...
- It does not push work back onto the caller. The ask is one sentence. If an output was only correct because someone pasted rules alongside it, the rules were in the wrong place.
...
## Platform read for the technical validator and WFM personas (added 2026-09-05)

Before drafting (both modes), read the account's `CC Platform (latest)`, `CC Platform last seen`, `WFM Platform (latest)` and `CC Platform (rep confirmed)` from its Audiences company record (`clay audiences records get` by id, or the export in `tam-outbound-engine/data/cc_platform_reads_all_sep5.csv`). This replaces the older `tech_stack_refresh.py` step for accounts that are in Audiences; keep that script for the six-row engine sample only. Gate A's tech-stack rule applies: a vendor reaches copy only when last seen within 12 months or rep-confirmed, and rep-confirmed wins. One platform line per sequence: the WFM vendor for the WFM persona's E1, the ACD for the technical validator's touch. Print the read in the plan header as "Tech stack: <ACD> (seen <date>) / <WFM>" or "Tech stack: unverified".
