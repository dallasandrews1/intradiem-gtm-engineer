...
- **Never a seller-want CTA.** "I would value / I'd value 15 minutes" is banned; the meeting must read as the prospect's payoff. Default natural patterns: "Thought it might be worth finding time to ___. Have 15 minutes ___?" and "Might be worth 15 minutes on ___. Open to it?"
...
- No clichés: "circle back," "touch base," "reach out," "at the end of the day," "trade notes," "trading notes," "compare notes," "comparing notes," "pick your brain" (use "talk through" / "walk through")
- Contractions always: I'd, I'm, you'll, that's. Uncontracted "I would" / "I am" in cold copy is an AI tell.
...
### Rule 8.5: Voice Audit (cadence gate, hard check parallel to the verified-claims gate)

Judge one thing only: does this read like a person typed it, or like AI generated it. This checks cadence, never facts (facts are the verified-claims gate's job). FAIL and rewrite if any is true:

- The first sentence packs two or more facts separated by commas (appositive stacking). FAIL example: "X sits at 3.0 stars this cycle, a full star under the bonus line, with no cliff-edge contract in the mix."
- Any sentence has two or more commas before its main verb.
- The copy uses a metaphor or decorative phrase for color: "the climb," "sequencing," "cliff-edge" in prose, "the ground you move," "spark," "north star," or a flourish sentence that adds no new fact.
...
PASS only if: one idea per sentence up front, varied length with at least one short sentence, plain words, no decorative phrases, and it sounds like a sharp person wrote it in one sitting. This is the SAME standard as the Clay MessageGen `voice_audit` critic column; keep the two identical so the human gate and the engine gate never drift. Cadence source of truth lives in intradiem-first-draft-engine (Gate 3, SENTENCE FLOW).
...
- [ ] Voice Audit passed: opener is one fact, no appositive stacking, no sentence with 2+ commas before its main verb
- [ ] At least one sentence under ~6 words; sentence lengths vary
- [ ] No decorative metaphors in prose ("the climb," "sequencing," "cliff-edge")
