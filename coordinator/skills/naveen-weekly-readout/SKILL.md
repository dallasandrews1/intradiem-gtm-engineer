...
- **Architecture storytelling.** When the readout or 1:1 prep touches how the engine itself works, pull the trigger/condition/action framing from `GTM_Swarm_Harmoniq_Framing.md` (project root): the engine runs the same architecture Harmoniq runs, applied to GTM signals. Lead with the parallel as a thread to shape together, never a walkthrough. Internal evt/chain log ids never appear in anything Naveen-facing.
