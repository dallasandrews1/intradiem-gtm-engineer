...
**Sep 5 demo table state:** Dallas built the demo view himself (Contact Center Platforms, Technologies Found, Contact Center Platform Latest formula all live); for the clip use rows where platforms/posts/Status are still blank (UI filter), not the five already-enriched rows; Email and email_final hold the same address on the demo rows, show Email. Why Now (2026-cycle) = formula keyed on Company from Dallas's public CMS 2026 Star Ratings analysis (cliff-edge contracts at 3.5 stars, forgone QBP, movable CS share), labeled public-data not Intradiem-verified.
