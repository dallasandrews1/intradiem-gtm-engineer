...
**Addendum (Sep 4):** `human_approved` is retired too. It was the copy-approval checkbox, and under the new model there is no Clay draft to approve; Nate reviews leads inside the lemlist campaign (lemlist's own reviewed-leads lock), which is the same control closer to the send and done by the person who owns the conversation. Clay keeps exactly two gates, both mechanical: verified email (`critic_email_valid`) and customer exclusion (`customer_exclude_bool`, never the text `customer_exclude`).
