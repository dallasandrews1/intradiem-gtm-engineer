...
**Lesson (Sep 4 late):** lemlist step delays are RELATIVE to the previous step, the page drawings use ABSOLUTE business days. The first build compounded them (Email 2 landed day 6 instead of day 3). Fixed: URL-path Call 1 delay 1, Email 2 delay 1, both Email 3 new-thread steps delay 2. Built arc now: E1 d0, visit d0, connect d1, Call 1 d2, E2 d3, accepted split d5, DM1 d5 / like d5, Call 2 + JTY d7, E3 d9, DM2 or visit d11, Call 3 d13, E4 d14, DM3 d16, breakup d18, withdraw d20. Always convert absolute to relative before add_sequence_step.

**v4 (Sep 4 late): drafts rewritten under the no-apology rule, multi-thread close live.** Variables colleagueFirst + colleagueLine on the 624 multi-seat leads (push_colleague_vars.py; lemlist REST returns 403 to Python's default User-Agent, add a UA header; POST /api/leads/{id}/variables creates, PATCH only updates existing). Liquid if-blocks confirmed to hide cleanly in preview when a variable is absent. Launch rule: account-complete days.

**Email 1 subject (Sep 4 late):** variant A subject "Quick idea to cut agent idle minutes at {{companyName}}" was Dallas's own change in the lemlist UI, on the lemlist agent's suggestion. Decided, not a flag. The A/B is a package test (subject + close); the Oct 9 read reports which package won.

**lemwarm (Sep 4 late, Dallas in the UI):** active on all mailboxes including both intradiemhq.com boxes. The UI-check gate is cleared; the time gate (three to four weeks of warm-up) still runs. Sender switch earliest Sep 21 to 25 depending on the start date.
