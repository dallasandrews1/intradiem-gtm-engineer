...
- Reproduced locally with Host = the Render hostname (13/13 smoke checks after the fix, 4 failures before). Not confirmed on Render itself: the key lives only in Render's GTM_API_KEYS. The Sep 1 Cowork strike-room "could not load accounts" is consistent with this and was attributed to the connector at the time ([[strike-room-cowork-connector-gap-sep1]]).
