...
- **REPORT-BACK block:** Polar has NO access to the Mac (probe Sep 13 2026: cloud sandbox only, honors stop lines). The sheet's last step says: end your final message with a block starting `REPORT <slug>` listing the named facts and the download links for any screenshots kept in Polar's workspace. Dallas pastes the block back; `automation/polar_intake.py --paste <slug>` logs it as a claim; downloaded files (optional) go to `automation/inbox/polar/<slug>/` or `~/Downloads/polar/<slug>/`. The task is registered in `automation/config/polar_tasks.json` with its verifier; the `polar-intake` agent performs the verifier read. Never write a sheet that asks Polar to save to a local path.
