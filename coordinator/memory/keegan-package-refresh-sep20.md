...
**Index rebuilt later Sep 20 (DEPLOYED, backoffice-maps 72734438). The three rooms got the same cuts and were DEPLOYED Sep 20 (save-rooms ddc92ea2) (hero counters and page description out, next move by owner in, engineering-process rules hidden via HIDE_RULES, Salesforce-known appendix off via SHOW_SF_KNOWN, shelf table open); the three sequence blueprint pages got the same cut and were DEPLOYED Sep 20 (Open points section off via SHOW_OPEN, status column only on doors and held, stale 'reads paused' and 'approved by hand' text removed, hero counts turned into jump links). See [[feedback-seller-pages-no-fluff]].** Dallas's standard for seller pages: nothing that states the obvious, no counters, no description of what a page is; every element must be something the rep acts on. The index now shows, per account, the next dated move per owner (Keegan first, computed from plans.py in the browser by the viewer's date), the 6sense stage, the reset date, one button and six links. Prior builder kept as `build_keegan_index.py.bak_sep20`. The room heroes still carry the same kind of counters (ways in, lanes, moves, leaders mapped) and are the next candidate.
