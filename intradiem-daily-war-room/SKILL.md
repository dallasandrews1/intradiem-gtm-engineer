...
- **Automated as of Jul 17 2026:** this runs unattended weekdays at 7:15am via a local launchd job (`automation/run_war_room.sh` in this project), logging to `automation/logs/war-room-<date>.md` and DMing Dallas on Slack. Dallas dropped his prior manual habit of running this by hand in Cowork each morning — don't proactively suggest or run a duplicate manual pass just because it's morning; the automated run already covers it. Still fine to trigger manually for a genuinely fresh reason (breaking news, pre-meeting check), just not as a default daily ritual.
...
4. **Keegan TAM watchlist (registered Aug 3 2026):** `motions/keegan/Keegan_TAM_Watchlist_Aug3.csv`, Keegan Sanders's prospect accounts. Sweep rows with status `watch` and `watch-unclassified` only. Rows marked `customer-excluded` (RBC, TIAA-CREF, Edward Jones) follow the install-base rule below. `needs-ruling` (RBC Insurance, a subsidiary of customer RBC) stays held until ruled. Thomas Jefferson University Hospital is `stars-overlap`; Stars routing wins there. Cardinal Health carries a note: a Jul 15 cost-mandate strike room already exists, coordinate before any new outreach. Signals on this universe route to the Keegan motion (`motions/keegan/Keegan_Attainment_Engine_Aug3.md`) and the seat ledger.
