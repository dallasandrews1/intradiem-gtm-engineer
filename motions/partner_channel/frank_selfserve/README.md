...
- Skill 3, `skills/partner-channel-landscape/` (Sep 2 2026): the channel map Frank asked for in the DM ("map out all of the channel partners bringing Five9, Cisco, Avaya, Genesys, Alvaria, Verint, and Amazon Connect to market in EMEA, who they resell and market share if possible"). One research pass per platform plus one for published share and distributors, one branded page with a filterable partner list, published-share table, platform cards, where-to-start. Worked example is the EMEA map built by `../build_landscape.py`. Frank runs it himself for the next region or platform set. Frank still owes Dallas his current partner-facing materials to seed the bank.
