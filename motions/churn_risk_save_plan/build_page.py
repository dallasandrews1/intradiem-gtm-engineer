...
<div><b data-n="3">3</b><span>accounts at risk</span></div>
<div><b data-n="89">89</b><span>Cleveland Clinic contacts in Salesforce</span></div>
<div><b data-n="16">16</b><span>names Inger found</span></div>
...
(function(){
  ...
  document.querySelectorAll('[data-n]').forEach(function(el){var t=+el.getAttribute('data-n'),s=0,st=Date.now();el.textContent='0';var iv=setInterval(function(){var p=Math.min(1,(Date.now()-st)/900);el.textContent=Math.round(t*(1-Math.pow(1-p,3)));if(p>=1)clearInterval(iv)},30)});
})();
</script>
"""
